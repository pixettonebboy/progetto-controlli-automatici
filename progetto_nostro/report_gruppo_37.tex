...
Possiamo definire $L(s)=R(s)\cdot G(s)$, questa viene chiamata funzione d'anello che rappresenta l’elemento fondamentale per l’analisi di stabilità e per la progettazione
...
\begin{align}
    F(s) &= \frac{L(s)}{1 + L(s)} && \text{(sensitività complementare),} \tag{16} \\
    S(s) &= \frac{1}{1 + L(s)} && \text{(sensitività),} \tag{17} \\
    Q(s) &= \frac{R(s)}{1 + L(s)} && \text{(sensitività di controllo).} \tag{18}
\end{align}

Applicando il principio di sovrapposizione degli effetti, si ottengono le relazioni:
\begin{align}
    Y(s) &= F(s)W(s) + S(s)D(s) - F(s)N(s), \tag{19} \\
    U(s) &= Q(s)W(s) - Q(s)D(s) - Q(s)N(s), \tag{20} \\
    E(s) &= S(s)W(s) - S(s)D(s) + F(s)N(s). \tag{21}
\end{align}
...
\begin{itemize}
    \item $S(s) = \frac{1}{1+L(s)}$: funzione di sensitività che modula l'errore e i disturbi.
    \item $T(s) = \frac{L(s)}{1+L(s)}$: funzione di sensitività complementare che modula il rumore di misura e l'inseguimento del riferimento.
\end{itemize}
???.
...
Per i sistemi del secondo ordine, la sovraelongazione percentuale $S\%$ in risposta a un gradino unitario è legata al coefficiente di smorzamento $\xi$ dalla relazione:
\begin{equation}
...
\begin{equation}
    \omega_n^{\min} = \frac{3}{\xi^* T_{s,5}} = \frac{3}{0.575 \cdot 0.01} \approx 5.22 \times 10^2 \text{ rad/s}.
\end{equation}

Si definisce pertanto:
\begin{equation}
    \omega_{c,\min} = \omega_n^{\min} \approx 5.22 \times 10^2 \text{ rad/s},
\end{equation}
