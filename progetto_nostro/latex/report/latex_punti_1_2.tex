...
La stabilità locale del sistema linearizzato può essere analizzata studiando gli autovalori della matrice A. In particolare, il sistema linearizzato è asintoticamente stabile se e solo se tutti gli autovalori di A hanno parte reale negativa. Nel caso in esame, gli autovalori della matrice \( A_e \)  si trovano risolvendo il polinomio caratteristico :
	\[
	\det(A_e - \lambda I) = 0.
	\]
...
La funzione di trasferimento \(G(s)\) è definita come la trasformata di Laplace della
risposta impulsiva del sistema, ovvero l’uscita generata in risposta a un ingresso impulsivo.
...
(sI - A_e)^{-1} =
\begin{bmatrix}
\dfrac{s + 0.95}{s^2 + 2.00s + 0.84} &
\dfrac{-0.45}{s^2 + 2.00s + 0.84} \\[0.3cm]
\dfrac{-0.35}{s^2 + 2.00s + 0.84} &
\dfrac{s + 1.05}{s^2 + 2.00s + 0.84}
\end{bmatrix}.
\label{eq:inv_matrix}
\end{equation}
...
D_e = 0,
\]
si ottiene la funzione di trasferimento della pianta:
\begin{equation}
...
\end{equation}
	...
	\end{equation}
    e rappresenta la variazione a regime della temperatura dell’aria in uscita in risposta a una variazione unitaria della potenza elettrica fornita al riscaldatore. 
