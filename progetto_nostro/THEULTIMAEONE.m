...
kappa  = 3e-3;        % coeff. non linearità resistenza (1/°C)
...
TR_e   = 175;                % temperatura resistenza in equilibrio
Tout_e = 80;                 % temperatura uscita aria in equilibrio
x_e    = [TR_e; Tout_e];

% Calcolo u_e imponendo dTR/dt = 0
KTR_e = 1 + kappa * TR_e;
u_e = -aR * (Tout_e - TR_e) * KTR_e;
...
if abs(f2_eq) > 1e-6
    fprintf('  ATTENZIONE: la seconda equazione non è in equilibrio.\n');
end
fprintf('\n');
...
if all(real(lambda) < 0)
    fprintf('→ Il sistema linearizzato è ASINTOTICAMENTE STABILE.\n');
else
    fprintf('→ Il sistema linearizzato NON è stabile.\n');
end
...
A_d      = 50;       % [dB] attenuazione disturbo (specifica formale)
A_n      = 60;       % [dB] attenuazione rumore (specifica formale)
...
fprintf('--- PUNTO 2: G(s) = ΔT_{out}(s) / ΔPE(s) ---\n\n');
...
fprintf('Osservazione: i poli di G(s) coincidono con gli autovalori della matrice A.\n');
fprintf('              Tutti i poli hanno parte reale negativa -> sistema BIBO stabile.\n\n');
...
fig = figure('Name','Punto 2 - Bode di G(s) con bande e vincoli','NumberTitle','off');
...
% Modulo di G(jω)
hG_mod = plot(ax1, wG, MagG_dB, 'LineWidth',1.5);
...
legend(ax1, [hG_mod, hDist_mod, hNoise_mod, hWc_mod],{'|G(j\omega)|', 'Banda disturbo', 'Banda rumore', '\omega_{c,min}'},'Location','southwest');
...
% Fase di G(jω)
hG_phase = plot(ax2, wG, PhaseG_deg, 'LineWidth',1.5);
...
% Zona "proibita" di fase per ω ≥ ω_c,min
...
legend(ax2, [hG_phase, hDist_phase, hNoise_phase, hZona_phase, hWc_phase], {'\angle G(j\omega)', 'Banda disturbo', 'Banda rumore', 'Zona proibita M_f', '\omega_{c,min}'}, 'Location','southwest');


%% ----------------- POLI E ZERI IN PIANO CARTESIANO -----------------

figure('Name','Punto 2 - Poli e zeri di G(s)','NumberTitle','off');
hold on; grid on; box on;

% Poli
plot(real(p), imag(p), 'kx', 'MarkerSize', 10, 'LineWidth', 1.5);

% Zeri (se presenti)
if ~isempty(z)
    plot(real(z), imag(z), 'ko', 'MarkerSize', 8, 'LineWidth', 1.5);
end
...
im_max = max(abs(imag(p)));
if im_max == 0
    im_max = 0.05;
end
ylim(1.2*[-im_max im_max]);

xlabel('Parte reale Re\{s\}','Interpreter','latex');
ylabel('Parte immaginaria Im\{s\}','Interpreter','latex');
title('Punto 2 -- Poli e zeri della funzione G(s)','Interpreter','latex');

if isempty(z)
    legend({'Poli'},'Location','best','Interpreter','latex');
else
    legend({'Poli','Zeri'},'Location','best','Interpreter','latex');
end
...
mag_Ge = abs(G_e_jw);
arg_Ge = rad2deg(angle(G_e_jw));   % fase in [−180,180] tipicamente
...
fprintf('  Ts (5%%)             = %.4e [s] (target <= %.4e [s])\n', info.SettlingTime, T_star);
fprintf('  Overshoot            = %.2f [%%] (target <= %.2f [%%])\n', info.Overshoot, Sovr_max*100);
fprintf('============================================================\n\n');
...
% 3.7 - Bode di L(s), S(s), T(s) con bande disturbo/rumore
figure('Name','Punto 3 - Bode di L, S, T con bande');
tlo = tiledlayout(2,1);
...
hL = plot(ax1, w, MagL_dB, 'LineWidth',1.6, 'DisplayName','|L(j\omega)|');
hS = plot(ax1, w, MagS_dB, 'LineWidth',1.4, 'DisplayName','|S(j\omega)|');
hT = plot(ax1, w, MagT_dB, 'LineWidth',1.4, 'DisplayName','|T(j\omega)|');

yl = ylim(ax1);

patch(ax1, [omega_plot_min, omega_dMAX, omega_dMAX, omega_plot_min],[yl(2), yl(2), yl(1), yl(1)],[0.8 0.8 0.8], 'FaceAlpha',0.3, 'EdgeColor','none', 'HandleVisibility','off');

patch(ax1, [omega_n_min, omega_n_max, omega_n_max, omega_n_min],[yl(2), yl(2), yl(1), yl(1)],[1 1 0], 'FaceAlpha',0.3, 'EdgeColor','none', 'HandleVisibility','off');

yline(ax1, -A_d, '--', sprintf('-A_d = -%.0f dB',A_d), 'HandleVisibility','off');
yline(ax1, -A_n, '--', sprintf('-A_n = -%.0f dB',A_n), 'HandleVisibility','off');

ylabel(ax1, 'Modulo [dB]');
title(ax1, 'Punto 3 - |L|, |S|, |T| con disturbo (grigio) e rumore (giallo)');
...
plot(ax2, w, PhaseL_deg, 'LineWidth',1.6, 'DisplayName','\angle L(j\omega)');
plot(ax2, w, PhaseS_deg, 'LineWidth',1.4, 'DisplayName','\angle S(j\omega)');
plot(ax2, w, PhaseT_deg, 'LineWidth',1.4, 'DisplayName','\angle T(j\omega)');

yl2 = ylim(ax2);

patch(ax2, [omega_plot_min, omega_dMAX, omega_dMAX, omega_plot_min], [yl2(2), yl2(2), yl2(1), yl2(1)],[0.8 0.8 0.8], 'FaceAlpha',0.2, 'EdgeColor','none', 'HandleVisibility','off');

patch(ax2, [omega_n_min, omega_n_max, omega_n_max, omega_n_min], [yl2(2), yl2(2), yl2(1), yl2(1)],[1 1 0], 'FaceAlpha',0.2, 'EdgeColor','none','HandleVisibility','off');

xlabel(ax2, '\omega [rad/s]');
ylabel(ax2, 'Fase [deg]');
title(ax2, 'Punto 3 - Fase di L, S, T');
legend(ax2,'Location','best');

tlo.TileSpacing = 'compact';
tlo.Padding     = 'compact';

% 3.8.1 Gradino di riferimento
figure('Name','Punto 3 - Risposta al gradino di riferimento');
plot(t_w, y_w,'LineWidth',1.5); grid on;
xlabel('Tempo [s]');
ylabel('y_w(t)');
title(sprintf('Risposta al gradino w(t) = %.1f·1(t)', W_max));

% 3.8.2 Disturbo
figure('Name','Punto 3 - Risposta al disturbo d(t)');
plot(t_sim, d_t, 'r--', t_sim, y_d, 'b','LineWidth',1.2); grid on;
legend('d(t)','y_d(t)');
xlabel('Tempo [s]'); ylabel('Ampiezza');
title('Punto 3 - Risposta al disturbo d(t)');

% 3.8.3 Rumore
figure('Name','Punto 3 - Risposta al rumore di misura n(t)');
plot(t_sim, n_t, 'r--', t_sim, y_n, 'b','LineWidth',1.2); grid on;
legend('n(t)','y_n(t)');
xlabel('Tempo [s]'); ylabel('Ampiezza');
title('Punto 3 - Risposta al rumore di misura n(t)');

% 3.8.4 Risposta totale
figure('Name','Punto 3 - Risposta totale y(t)');
hold on; grid on;
plot(t_sim, y_w, 'r--','LineWidth',1.2);
plot(t_sim, y,   'b','LineWidth',1.5);
legend('Solo riferimento y_w(t)', 'y(t) totale');
xlabel('Tempo [s]'); ylabel('Ampiezza');
title('Punto 3 - Risposta totale y(t)');

patch([0, T_sim, T_sim, 0], [LV*(1+Sovr_max), LV*(1+Sovr_max), LV*2, LV*2], 'r','FaceAlpha',0.2,'EdgeAlpha',0.4);
patch([T_star, T_sim, T_sim, T_star],[LV*(1-0.05), LV*(1-0.05), 0, 0], 'g','FaceAlpha',0.1,'EdgeAlpha',0.3);
...
patch(ax1, [omega_plot_min omega_dMAX omega_dMAX omega_plot_min],[yl(2) yl(2) yl(1) yl(1)], [0.74 0.93 1], 'FaceAlpha',0.35, 'EdgeColor','none');
patch(ax1,[omega_n_min omega_n_max omega_n_max omega_n_min], [yl(2) yl(2) yl(1) yl(1)], [1 1 0.75], 'FaceAlpha',0.35, 'EdgeColor','none');
patch(ax1, [omega_c_min w_plot(end) w_plot(end) omega_c_min], [yl(2) yl(2) yl(1) yl(1)], [1 0.9 0.5], 'FaceAlpha',0.35, 'EdgeColor','none');

plot(w_plot, MagGe_dB, 'Color',[0 0.45 0.74], 'LineWidth',1.7);
plot(w_plot, MagL2_dB , 'Color',[0.85 0.33 0.10], 'LineWidth',1.7);
plot(wc, 0, 'o', 'MarkerSize',7, 'LineWidth',1.2, 'Color',[0 0.45 0.74]);
...
patch(ax2,[omega_c_min Wpm Wpm omega_c_min], [yl2(2) yl2(2) yl2(1) yl2(1)],  [1 0.8 0.85], 'FaceAlpha',0.35, 'EdgeColor','none');

plot(w_plot, PhaseGe_deg2,'Color',[0 0.45 0.74], 'LineWidth',1.7);
plot(w_plot, PhaseL_deg2 ,'Color',[0.85 0.33 0.10], 'LineWidth',1.7);
plot(Wpm, phase_at_Wpm, 'o', 'MarkerSize',7, 'LineWidth',1.2, 'Color',[0.93 0.69 0.13]);

yline(-180,'Color',[0.4 0.4 0.4]);
xlabel('Frequenza (rad/s)');
ylabel('Fase (deg)');

dummy_dist = patch(nan,nan,[0.74 0.93 1],'FaceAlpha',1,'EdgeColor','none');
dummy_rum  = patch(nan,nan,[1 1 0.75],'FaceAlpha',1,'EdgeColor','none');
dummy_wc   = patch(nan,nan,[1 0.9 0.5],'FaceAlpha',1,'EdgeColor','none');
dummy_Mf   = patch(nan,nan,[1 0.8 0.85],'FaceAlpha',1,'EdgeColor','none');
dummy_wc_pt = plot(nan,nan,'o','Color',[0 0.45 0.74],'MarkerSize',7, 'LineWidth',1.2);
dummy_Mf_pt = plot(nan,nan,'o','Color',[0.93 0.69 0.13], 'MarkerSize',7, 'LineWidth',1.2);

legend([ 
        plot(nan,nan,'Color',[0 0.45 0.74],'LineWidth',1.7),plot(nan,nan,'Color',[0.85 0.33 0.10],'LineWidth',1.7), dummy_dist, dummy_rum, dummy_wc, dummy_Mf, dummy_wc_pt, dummy_Mf_pt], ...
       { 'G_e(s)', 'L(s)', 'Banda disturbo', 'Banda rumore',  '\omega_c \ge \omega_{c,min}', sprintf('M_f \\ge %.2f°', PM), '\omega_c', 'Intersezione M_f'},'Location','eastoutside');
