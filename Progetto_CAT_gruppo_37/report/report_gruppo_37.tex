...
\usepackage{siunitx} %per notazione scientifica
...
\begin{document}
	...
	\begin{align}
		...
		m_A c_A \frac{dT_{\text{out}}(t)}{dt} &= \dot{m}_A c_A (T_{\text{in}} - T_{\text{out}}(t))+ h_R A_R (T_R(t) - T_{\text{out}}(t)). \label{eq:diff2}
	\end{align}
	...
	\begin{tcolorbox}[
		...
		A partire dai valori di equilibrio \(T_{\text{R,e}}\) e \(T_{\text{out},e}\) (forniti in tabella), si trovi l’intera coppia di equilibrio \((x_e, u_e)\) e si linearizzi il sistema non lineare (\ref{eq:sistema}) nell’equilibrio, così da ottenere un sistema linearizzato del tipo
		\[
	...
	\begin{equation}
		...
		{ m_A c_A }.
	\end{equation}
	dove si è posto il coefficiente di scambio termico \( a_R \coloneqq h_R A_R \).
	...
	\begin{itemize}
		...
		\item \textbf{Strettamente proprio}, poichè $y(t)=T_{\text{out}}(t)$, quindi l'uscita non dipende dall'ingresso, ma solo dallo stato del sistema .
	\end{itemize}
	
	\subsection{Coppia di equilibrio}
	Dato un sistema tempo invariante continuo di tipo $\dot{x}(t) = f(x(t),u(t))$, abbiamo che \((x_e, u_e)\) è una \textit{coppia di equilibrio} se $f(x_e, u_e)=0$.
	...
	bilancio termico in cui la potenza elettrica fornita al riscaldatore compensa le perdite di calore dovute allo scambio con l’aria.
	...
	Definiamo quindi lo \textbf{stato di equilibrio} come:
	...
	L’\textbf{ingresso di equilibrio} \(u_e\) può essere determinato imponendo l’annullamento
	...
	definendo la costante $K_{T_R,e} \, = \, 1+ \kappa \, T_{\text{R,e}}$, otteniamo l'ingresso di equilibrio:
	
	\begin{equation}
		0 = h_R A_R \left( T_{\text{out},e} - T_{\text{R,e}} \right)
		+ \frac{u_e}{1 + \kappa T_{\text{R,e}}} .
	\end{equation}
	Da cui segue:
	\begin{equation}
		u_e = P_{E,e}= -\, a_R \, (T_{\text{out},e} - T_{\text{R,e}}) \, K_{T_R,e}
	\end{equation}
	\begin{equation}
	...
	Per verificare che \((x_e, u_e)\) è una coppia di equilibrio, valutiamo le equazioni nel punto $\left(T_{\text{R,e}},\, T_{\text{out},e},\, u_e\right)$:
	
	\[
	f_1(T_{\text{R,e}}, T_{\text{out},e}, u_e), \qquad
	f_2(T_{\text{R,e}}, T_{\text{out},e}, u_e).
	\]
	...
	\begin{equation}
		f_1(T_{\text{R,e}}, T_{\text{out},e}, u_e) = 0 \; \text{[°C/s]},
		\qquad
		f_2(T_{\text{R,e}}, T_{\text{out},e}, u_e) = 9.7 \cdot 10^{-5}\ \; \text{[°C/s]}.
	\end{equation}
	...
	viene linearizzato mediante uno \textit{sviluppo di Taylor} al primo ordine.
	...
	Le matrici \(A\), \(B\), \(C\) e \(D\) rappresentano rispettivamente le \textit{Jacobiane} del modello
	...
	Il calcolo numerico delle Jacobiane tramite differenze finite è giustificato dalla regolarità delle funzioni di stato. Il passo $\Delta$ è scelto sufficientemente piccolo in modo da garantire una buona approssimazione delle derivate parziali. 
	...
	Da $A_{11}$ capiamo che la dinamica di $T_R$ è lenta; da $A_{22}$ capiamo che la dinamica di $T_{\text{out}}$ è veloce.
	
	La \textbf{matrice d’ingresso} è:
	\begin{equation}
	...
	Poiché entrambi presentano parte reale negativa, il sistema linearizzato risulta \textit{asintoticamente stabile} nell’intorno del punto di equilibrio considerato. Si osserva inoltre che i due autovalori sono separati di diversi ordini di grandezza. Il polo $\lambda_1$ è associato a una dinamica lenta, mentre il polo $\lambda_2$ descrive una dinamica più rapida 
	\[
	...
	è possibile definire la \textit{funzione di trasferimento} $G(s)$ come il rapporto tra la trasformata di Laplace dell’uscita e quella dell’ingresso, assumendo condizioni iniziali nulle $x(0)=0$.
	...
	\begin{equation}
		G(s)=\frac{\Delta T_{\text{out}}(s)}{\Delta PE(s)}
	\end{equation}
	...
	mentre $T_{\text{out}}$ è influenzata indirettamente tramite lo scambio termico.
	Trasferimento del sistema:
	\begin{equation}
		G(s) = \frac{1.03 \cdot 10^{-5}}{s^2 + 1.9566s + 0.0041}.
		\label{eq:tf_pianta}
	\end{equation}
	
	Ponendo \(s = j\omega\), la funzione di trasferimento in frequenza assume la forma:
	\begin{equation}
		G(j\omega) = \left. \frac{1.03 \cdot 10^{-5}}{s^2 + 1.9566s + 0.0041} \right|_{s = j\omega}.
		\label{eq:tf_freq}
	\end{equation}
	
	
	\subsubsection{Guadagno statico}
	Definiamo il \textit{guadagno statico} della pianta come:
	...
	Tutti i poli presentano parte reale negativa; il sistema è quindi \textbf{BIBO stabile}.
	...
	\begin{figure}[H]
		...
		\caption{Poli e zeri di $G(s)$.}
		\label{fig:Punto2-Poli_e_Zeri_di_G(s)}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\caption{Diagramma di Bode di $G(s)$.}
		\label{fig:Punto2-Bode_di_G(s)}
	\end{figure}
	...
	Progettiamo un \textbf{regolatore} $R(s)$ per l'impianto $G(s)$ in modo che le specifiche vengano rispettate.
	...
	Possiamo definire $L(s)=R(s)\cdot G(s)$, questa viene chiamata \textbf{funzione d'anello} che rappresenta l’elemento fondamentale per l’analisi di stabilità e per la progettazione
	del sistema di controllo in frequenza.
	Il sistema è soggetto alla presenza dei seguenti segnali:
	\begin{itemize}
		\item $\omega(t)$: Ingresso di riferimento.
		\item $y(t)$: Uscita del sistema.
		\item $e(t)=y(t)-\omega(t)$: Errore tra l'uscita ed il riferimento.
		\item $d(t)$: Disturbo agente sull'uscita.
		\item $n(t)$: Rumore di misura.
		
	\end{itemize}
	\bigskip
	Prima di continuare definiamo alcuni concetti fondamentali:
	\subsubsection*{Pulsazione critica $\omega_c$}
	La pulsazione critica $\omega_c$ è definita come la pulsazione alla quale il modulo della funzione
	d’anello attraversa il valore unitario:
	\begin{equation} |L(j\omega_c)|_{dB} = 0 \text{ dB} \end{equation}
	
	\subsubsection*{Pulsazione $\omega_\pi$}
	È la pulsazione alla quale la fase di $L(j\omega)$ assume il valore di -180°:
	\begin{equation} arg(L(j\omega_{\pi})) = -180^\circ \end{equation}
	
	\subsubsection*{Margine di fase}
	fornisce una misura della robustezza del sistema in termini di
	stabilità ed è definito come:
	\begin{equation} M_f = arg(L(j\omega_c)) - (-180^\circ) = arg(L(j\omega_c)) + 180^\circ \end{equation}
	
	\subsubsection*{Margine di ampiezza}
	quantifica la tolleranza del sistema a variazioni del guadagno:
	\begin{equation} M_a = -|L(j\omega_\pi)|_{dB} \end{equation}
	
	\subsubsection*{Criterio di Bode}
	Si supponga che:
	\begin{enumerate}
		\item $L(s)$ non presenti poli a parte reale positiva;
		\item il modulo $|L(j\omega)|$ attraversi una sola volta il valore unitario,
	\end{enumerate}
	allora il sistema retroazionato è asintoticamente stabile se e solo se entrambi i margini risultano positivi. 
	\\Per analizzare l'effetto del riferimento, dei disturbi e del rumore di misura, si introducono le seguenti funzioni di sensitività:
	
	
	\begin{align}
		L(s) &=R(s)\cdot G(s) && \text{(funzione d'anello),} \\
		F(s) &= \frac{L(s)}{1 + L(s)} && \text{(funzione di trasferimento),} \\
		S(s) &= \frac{1}{1 + L(s)} && \text{(funzione di sensitività),} \\
		Q(s) &= \frac{R(s)}{1 + L(s)} && \text{(sensitività di controllo),} \\
		T(s) &= \frac{L(s)}{1 + L(s)} && \text{(funzione di sensitività complementare).}
	\end{align}
	...
	Nel nostro caso $F(s)=T(s)$ a causa del sistema in retroazione.
	
	
	Per studiare ogni riferimento, applichiamo il principio di sovrapposizione degli effetti, si ottengono le relazioni:
	\begin{align}
		Y(s) &= F(s)W(s) + S(s)D(s) - F(s)N(s), \\
		U(s) &= Q(s)W(s) - Q(s)D(s) - Q(s)N(s), \\
		E(s) &= S(s)W(s) - S(s)D(s) + F(s)N(s). 
	\end{align}
	L’obiettivo ideale del controllo è ottenere un’uscita che segua il riferimento e sia poco
	sensibile ai disturbi e al rumore di misura. In termini di risposta in frequenza, ciò si
	traduce nelle seguenti condizioni desiderabili:
	\begin{itemize}
		\item $|F(j\omega)| \approx 1$ e $|S(j\omega)| \approx 0$ alle basse frequenze;
		\item $|F(j\omega)| \approx 0$ alle alte frequenze.
	\end{itemize}
	
	
	\bigskip
	L'errore tra l'uscita ed il riferimento è definito come:
	\begin{equation}e(t) = y(t) - \omega(t) \end{equation}
	...
	\begin{figure}[H]
		...
		\caption{Diagramma di Bode di $L(s)$,$S(s)$,$T(s)$.}
		\label{fig:Punto3-Bode_L_S_T}
	\end{figure}
	...
	\begin{itemize}
		...
		\end{itemize}
		\item Specifiche derivate (che andremo a calcolare tra poco)
		\begin{itemize}
			\item Smorzamento $\xi^*$.
			\item Margine di fase desiderato $M_f$
			\item Pulsazione crossover $\omega_{c,min}$
		\end{itemize}
	\end{itemize}
	\subsubsection{Specifica sulla sovraelongazione}
	
	Per i sistemi del secondo ordine, la sovraelongazione percentuale $S\%$ in risposta a un gradino unitario è legata al \textit{coefficiente di smorzamento} $\xi$ dalla relazione:
	\begin{equation}
		S\% = 100 e^{-\frac{\pi\xi}{\sqrt{1-\xi^2}}}.
	\end{equation}
	
	Invertendo tale relazione è possibile ricavare il valore minimo di smorzamento richiesto in funzione della sovraelongazione massima ammissibile:
	\begin{equation}
		\xi = \frac{-\ln \left( \frac{S\%}{100} \right)}{\sqrt{\pi^2 + \ln^2 \left( \frac{S\%}{100} \right)}}.
	\end{equation}
	
	Nel presente progetto è richiesta una sovraelongazione percentuale massima pari a:
	\begin{equation}
		S\% \leq S^* = 11\%. 
	\end{equation}
	
	
	Sostituendo tale valore nella relazione precedente si ottiene:
	\begin{equation}
		\xi^* \approx 0.575.
	\end{equation}
	
	Per sistemi del secondo ordine è possibile stabilire una relazione approssimata tra il coefficiente di smorzamento $\xi$ e il margine di fase $M_f$, data da:
	\begin{equation}
		\xi \approx \frac{M_f}{100}.
	\end{equation}
	
	Da cui segue che il margine di fase minimo richiesto per soddisfare la specifica sulla sovraelongazione è:
	\begin{equation}
		M_{f,\min} \approx 100 \, \xi^* \approx 57.5^\circ.
	\end{equation}
	
	Si osserva che tale valore risulta più restrittivo rispetto al margine di fase minimo imposto dalla specifica di stabilità, risultando quindi determinante nella successiva progettazione del regolatore dinamico.
	\subsubsection{Specifica sul tempo di assestamento}
	
	Il tempo di assestamento rappresenta un indicatore fondamentale delle prestazioni dinamiche del sistema. In generale, il tempo di assestamento $T_{a,\epsilon}$ è legato alla costante di tempo del sistema e all'errore relativo ammesso $\epsilon$ dalla relazione:
	\begin{equation}
		T_{a,\epsilon} = T \ln \left( \frac{1}{0.01 \epsilon} \right).
	\end{equation}
	
	Nel caso di sistemi del secondo ordine, il tempo di assestamento al 5\% può essere approssimato dalla ben nota relazione:
	\begin{equation}
		T_{s,5} \approx \frac{3}{\xi \omega_n},
	\end{equation}
	dove $\xi$ è il coefficiente di smorzamento e $\omega_n$ è la pulsazione naturale del sistema. Nel presente progetto è richiesto che il tempo di assestamento soddisfi la specifica:
	\[
	T_{s,5} \leq 0.01 \text{ s}.
	\]
	
	Utilizzando il valore di smorzamento ricavato dalla specifica sulla sovraelongazione, $\xi^* \approx 0.575$, è possibile stimare una pulsazione naturale minima:
	\begin{equation}
		\omega_n^{\min} = \frac{3}{\xi^* T_{s,5}} = \frac{3}{0.575 \cdot 0.01} \approx 5.22 \cdot 10^2 \text{ rad/s}.
	\end{equation}
	
	Si definisce pertanto:
	\begin{equation}
		\omega_{c,\min} = \omega_{n,\min} \approx 5.22 \cdot 10^2 \text{ rad/s},
	\end{equation}
	che rappresenta l'estremo inferiore dell'intervallo all'interno del quale dovrà essere posizionata la pulsazione di attraversamento $\omega_c$ nella successiva progettazione del regolatore dinamico.
	
	Riassumendo, per quanto riguarda le specifiche derivate, abbiamo come risultati:
	\begin{equation}
		\xi^\star\approx0.575 \ , \qquad M_f\approx57.5^\circ \ , \qquad \omega_{c,min}=521.8\frac{rad}{s}.
	\end{equation}
	
	\subsection*{Sintesi regolatore}
	Il regolatore è realizzato come \textbf{combinazione tra la sua parte statica e quella dinamica} $R(s)=R_s(s)\cdot R_d(s)$.
	
	\subsection{Regolatore statico $R_s(s)$}
	Il regolatore statico ha il compito di soddisfare le specifiche relative al comportamento a regime del sistema, in particolare per quanto riguarda l'errore stazionario in risposta a un riferimento costante e l'attenuazione dei disturbi a bassa frequenza. In generale, considerando un riferimento a gradino $W(s) = W/s$, l'errore a regime è dato da:
	
	\begin{equation}
		e_{\infty} = \lim_{t \to \infty} e(t) = \lim_{s \to 0} sE(s) = \lim_{s \to 0} s S(s) W(s) = W \lim_{s \to 0} S(s),
	\end{equation}
	
	\vspace{10pt}
	\noindent dove $S(s) = \frac{1}{1+L(s)}$ è la funzione di sensitività. Nel caso di un sistema privo di poli nell'origine, l'errore a regime non è nullo, ma può essere reso sufficientemente piccolo aumentando opportunamente il guadagno d'anello alle basse frequenze. In particolare, indicando con $\mu = \lim_{s \to 0} L(s)$ il guadagno statico della funzione d'anello, si ottiene:
	
	\begin{equation}
		e_{\infty} = \frac{W}{1 + \mu}.
	\end{equation}
	
	Nel presente progetto non si introduce un polo nell'origine nel regolatore. Di conseguenza, il regolatore statico è scelto come un semplice guadagno costante:
	
	\begin{equation}
		R_s(s) = R_s.
	\end{equation}
	
	Questa scelta consente di ridurre l'errore a regime a un valore sufficientemente contenuto, senza introdurre gli effetti collaterali tipici dell'azione integrativa, quali un eccessivo sfasamento e una maggiore sensibilità al rumore alle alte frequenze. Il valore del guadagno statico $R_s$ viene determinato imponendo il soddisfacimento delle specifiche sull'errore a regime e sull'attenuazione del disturbo in uscita nella banda di bassa frequenza. Tra i valori ottenuti, viene selezionato quello più restrittivo, così da garantire il rispetto simultaneo di tutte le specifiche assegnate.
	...
	\begin{itemize}
		...
		\item Ridurre il guadagno alle alte frequenze (finalizzato l'attenuazione del rumore $\ge 60\ dB$ in $[\omega_{n,min}\ ,\ \omega_{n,max}]$, con $\omega_{n,min} = 8\cdot10^4$ e $\omega_{n,max} = 9\cdot10^6$).
	...
	\begin{figure}[H]
		...
		\caption{Diagramma di bode di $L(s)$ e $G(s)$.}
		\label{fig:Punto3-Bode_L(s)_Ge(s)}
	\end{figure}
	...
	\paragraph{Precisazioni sul nostro regolatore:}
	Il guadagno statico garantisce l'accuratezza a regime mentre la rete anticipatrice migliora il margine di fase e la rapidità della risposta. L'introduzione di un integratore non è stata necessaria poiché l' errore a regime risulta abbastanza piccolo e un polo nell'origine avrebbe peggiorato la robustezza del sistema portando alla non soddisfazione di alcuna specifica. La struttura del regolatore è stata scelta in modo che tutte le specifiche progettuali fossero soddisfatte contemporaneamente.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.95\textwidth]{figs/Punto3-Bode_di_L(s).pdf}
		\caption{Diagramma di Bode di $L(s)$.}
		\label{fig:Punto3-Bode_di_L(s)}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\caption{Ingressi nello scenario breve.}
		\label{fig:Punto4-Scenario_breve_(Ingressi)}
	\end{figure}
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.95\textwidth]{figs/Punto4-Scenario_lungo_(Ingressi).pdf}
		\caption{Ingressi nello scenario lungo.}
		\label{fig:Punto4-Scenario_lungo_(Ingressi)}
	\end{figure}
	...
	Avremo che:
	\begin{equation}
		e_\infty=W_{max}|1-T(0)|=W_{max}|S(0)|
	\end{equation}
	...
	\begin{itemize}
		...
		\item L'uscita mostra un'ampiezza molto più piccola confermando una forte attenuazione.
	\end{itemize}
	...
	\begin{figure}[H]
		...
		\caption{Uscite totali.}
		\label{fig:Punto4-Uscite_totali}
	\end{figure}
	...
	Il sistema di controllo è stato testato sul modello non lineare completo, in presenza di riferimento, disturbo e rumore di misura, verificando che per condizioni iniziali e ampiezze del riferimento compatibili con l’intorno considerato le traiettorie di stato e uscita del sistema in anello chiuso rimangono nell’intorno del punto di equilibrio, con variazioni limitate rispetto ai valori di equilibrio, confermando la validità locale della linearizzazione e l’efficacia del regolatore progettato.
	\\Per lo svolgimento di questo punto abbiamo utilizzato \textit{Simulink}.
	...
	\begin{figure}[H]
		...
		\caption{Modello del nostro sistema realizzato con Simulink.}
		\label{fig:Punto5-Modello_Simulink}
	\end{figure}
	...
	\begin{itemize}
		...
		\item $y(t)$ : Blocco scope che mostra l'uscita non lineare.
	\end{itemize}
	
	\subsubsection{Simulazione con disturbi applicati}
	Lo scope all'uscita risulterà: 
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.95\textwidth]{figs/Punto5-Scope_y.pdf}
		\caption{Rappresentazione grafica dello scope all'uscita generata da Simulink.}
		\label{fig:Punto5-Scope_y}
	\end{figure}
	\clearpage
	
	\section*{Conclusioni}
	In conclusione facciamo una sintesi di tutti i passaggi più importanti che abbiamo effettuato.
	
	\paragraph{Matrici per la linearizzazione intorno a un punto di equilibrio:}Dopo aver analizzato il tipo di sistema fornito abbiamo trovato le matrici utili alla scrittura
	in forma linearizzata intorno al punto di equilibrio $(x_e,u_e)$, le quali sono:
	\[
	A_e =
	\begin{bmatrix}
		-\num{2.1e-3} & \num{1.6e-3} \\
	...
	\paragraph{Autovalori:}Abbiamo calcolato gli autovalori della matrice Ae verificando l’asintotica stabilità del sistema, essendo essi a parte reale negativa.
	\[
	\lambda_1 = -\num{2.1e-3}, \qquad
	\lambda_2 = -1.9545.
	\]
	
	\paragraph{Funzione di trasferimento $G(s)$:}Trovando la funzione di trasferimento abbiamo confermato la BIBO stabilità del sistema.
	Inoltre tramite la rappresentazione dei diagrammi di Bode abbiamo potuto analizzare l’andamento della $G(j\omega)$, come possiamo vedere nella Figura~\ref{fig:Punto2-Bode_di_G(s)} ed abbiamo analizzato i suoi poli e zeri, come possiamo vedere nella Figura~\ref{fig:Punto2-Poli_e_Zeri_di_G(s)}.
	\[
	G(s) = \frac{1.03 \cdot 10^{-5}}{(s + 1.9545)(s + 0.0021)}.
	\]
	
	\paragraph{Specifiche del regolatore:}Abbiamo studiato le specifiche ottenendo valori da soddisfare tramite la sintesi del regolatore, il quale è definito come $R(s)=R_s\cdot R_d(s)$ dove $R_s$ è la sua parte statica (per l'errore a regime) e $R_d(s)$ la sua parte dinamica (per la sovraelongazione percentuale, il tempo di assestamento e
	l'attenuazione dei disturbi).
	\begin{itemize}
		\item $|e_{\infty}| \le 0.002$.
		\item $\xi^\star\approx0.575$
		\item $ M_f\approx57.5^\circ$
		\item $\omega_{c,min}=521.8\frac{rad}{s}.$
	\end{itemize}
	
	\paragraph{Rete anticipatrice e regolatore finale:}Abbiamo utilizzato una rete anticipatrice e un polo ad alta frequenza per realizzare la $L(s)$ ottimale in risposta ai segnali $\omega(t)$, $d(t)$ e $n(t)$. 
	\[
	R(s)=R_s\cdot R_d(s)
	=
	2.47\cdot10^{7}\;
	...
	\paragraph{Simulazione sul modello non lineare:}Il sistema di controllo è stato testato sul modello non lineare in presenza di riferimento, disturbo e
	rumore di misura, tramite l'utilizzo di Simulink.
\end{document}
